...
REDUCERS[Tokens.TEXT] = (node, token) => {
  ...
  if (!node.is(Nodes.TEXT)) {
    token.text = token.text.trimLeft()
  }

  if (token.next.is(Tokens.NEWLINE)) {
    token.text = token.text.trimRight()
  }
  ...
  let text = new Node(Nodes.TEXT, token.text)
  node.addChild(text)
...
REDUCERS[Tokens.STAR] = (node, token) => {
  ...
  if (node.parent.is(Nodes.LI)) {
    if (token.previous.is(Tokens.NEWLINE)) {
